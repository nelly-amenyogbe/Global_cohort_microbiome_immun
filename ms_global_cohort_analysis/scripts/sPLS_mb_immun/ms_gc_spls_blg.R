...
# In this script, we perform sPLS for BELGIAN children. We then refine the model to only include features that vary across at least 30% of features in the respective data frame, and plot selected examples alongside their univariate correlation strength.
...
# Plot ordinations
plotIndiv(spls.res, comp = c(1,3)) # comp1 - comp2 show dispersion
...
cor.comp.12 + facet_grid(~Data) # appears that comp1 and 2 both have correlation structure
...
hm.est(lm.plot, 
       x.vars = x.feats,
       y.vars = y.feats,
       title = "Belgium comp1:2",
       num.display = FALSE,
       spls.meta = spls.meta)

# saved as sPLS_mb_immun/blg/ms_gc_spls_blg_hm.png

# save data ####
blg.spls.save <- list("spls.obj" = spls.res, "otu1.select" = sum1.sX$X.var, "otu2.select" = sum2.sX$X.var, "lmx1.select" = sum1.sY$Y.var, "lmx2.select" = sum2.sY$Y.var)

#saveRDS(blg.spls.save, "ms_global_cohort_analysis/Rdata/R_export/spls_res/blg_spls_res.rds")
...
#ggsave("ms_global_cohort_analysis/figures/sPLS-mb_immun/blg/blg_cor_circle_sig.pdf", device = "pdf", dpi = 300, width = 7.2, height = 4.8)
...
# plot
p <- plot(net12$net, layout = lo, main = "Belgium Comp1:2")
...
# saved as ms_global_cohort_analysis/figures/sPLS-mb_immun/blg/ms_gc_spls_blg_rn.png

# save network
#write_graph(net12$net, "ms_global_cohort_analysis/figures/sPLS-mb_immun/blg/blg_rn12.graphml", format = "graphml")

# plot selected relationships ####

dat <- lm.dat

dat.p <- filter(dat, X.var %in% c("OTU_101 F_Lachnospiraceae", "OTU_59 G_Bacteroides"),
                Y.var %in% c("IL-1b_PAM", "IL-6_PAM"))

p.blg <- ggplot(dat.p, aes(x = Y.value, y = X.value)) +
  geom_point(size = 2, shape = 19, color = "#fec44f") +
...
#ggsave("ms_global_cohort_analysis/figures/sPLS-mb_immun/blg/blg_example_plots.pdf", device = "pdf", dpi = 300, width = 11, height = 2.3)

# get blg only res
blg.res <- x.y.lm.scaled(dat.p)
blg.res <- blg.res[,c("Estimate", "p.value", "X.var", "Y.var")]
#write.csv(blg.res, "ms_global_cohort_analysis/figures/sPLS-mb_immun/blg/blg_example_plot_annotations.csv")
